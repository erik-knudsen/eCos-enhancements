...
**
** $Id: os_unix.c,v 1.2 2009/12/30 14:59:27 erik Exp $
*/
#include "sqliteInt.h"
...
static int semClose(sqlite3_file *id) {
  if( id ){
    ...
    releaseLockInfo(pFile->pLock);
    releaseOpenCnt(pFile->pOpen);
    unixLeaveMutex();	//EAK: Moved from after closeUnixFile, since vxWorksReleaseFileId acquires this mutex.
    closeUnixFile(id);
  }
  return SQLITE_OK;
}
...
#if defined(SQLITE_PREFER_PROXY_LOCKING) && SQLITE_PREFER_PROXY_LOCKING
