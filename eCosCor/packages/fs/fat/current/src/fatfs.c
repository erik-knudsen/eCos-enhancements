...
fatfs_fo_write(struct CYG_FILE_TAG *fp, struct CYG_UIO_TAG *uio)
{
	...
	//Fill hole? Extend file if needed and write zeroes in hole.
	if (pos > node->dentry.size)
	{
		char *buf, *tmp;
		off_t len;
		len = pos - node->dentry.size;

		buf = tmp = calloc(len, 1);
		if (buf == NULL)
			return EINVAL;
...
fatfs_fo_lseek(struct CYG_FILE_TAG *fp, off_t *apos, int whence)
{
    ...
    if (pos > fd->node->dentry.size)	//Set to eof in preparation for file hole.
		err = fatfs_setpos(disk, &fd->node->dentry, &fd->pos, fd->node->dentry.size);
	else
